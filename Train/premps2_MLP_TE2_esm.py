...
for epoch in range(start_epoch, num_epochs):
    ...
    for i_batch, sample in enumerate(train_dataloader):
        ...
        optimizer.step()
        scheduler.step()
        epoch_train_loss += loss.item() * mutation.size(0)
    ...
    with torch.no_grad():
        ...
        for sample in valid_dataloader:
            ...
            pred = torch.squeeze(logits, 1)
            loss = loss_fn(pred, labels)
            epoch_valid_loss += loss.item() * mutation.size(0)
...
np.save(f'{path_save}/train_loss.npy',a)
b = np.array(valid_losses)
np.save(f'{path_save}/valid_losses.npy',b)
